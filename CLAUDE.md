...
실제 중식 배달 매장 도입을 준비하는 단계다(README 와 같은 기준). 가상의 사용자가 아니라 이 화면을 매일 쓸 사장님이 정해져 있고, 화면이 깨지면 그날 장사에 영향이 간다.
...
> 이 절은 쉽게 낡는다. 고칠 때는 **워킹트리가 아니라 `origin/main` 을 보고** 확인한다. 한 번은 낡은 로컬 main(`37f6515`)에서 브랜치를 따는 바람에 이미 끝난 문제를 다시 고치는 PR(#52)이 만들어졌고, 같은 기준에서 딴 #50 #51 은 main 보다 30여 커밋 뒤에 있다. 마지막 확인: 2026-09-25, `57cab27`.
