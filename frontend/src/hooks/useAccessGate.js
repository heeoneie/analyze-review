...
import { getAuthConfig, getMe, getReplyConfig, getAccessCode } from '../api/client';
...
export default function useAccessGate() {
  ...
  const open = useCallback(
    (required) => setGate((prev) => ({ status: 'open', required: required ?? prev.required })),
    [],
  );

  // 요청이 401 로 튕겼을 때 다시 잠근다.
  const lock = useCallback(() => setGate({ status: 'locked', required: true }), []);
